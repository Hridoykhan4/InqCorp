...
const testimonials = [
  {
    ...
    company: "Narayanganj knit factory",
    rating: 5,
    text: "We ordered 40 hose cabinets and fire-rated doors for a compliance audit. SafetyPlus gave us the datasheets up front, delivered on schedule, and the build quality passed inspection first time.",
  },
  {
    name: "Facilities Manager",
    title: "Commercial High-Rise",
    company: "Dhaka office complex",
    rating: 5,
    text: "Sourcing DB boxes, racks and signage used to mean five suppliers. Getting it from one local manufacturer cut our lead time and made the paperwork far simpler.",
  },
  {
    name: "Safety Officer",
    title: "Pharmaceutical Plant",
    company: "Gazipur facility",
    rating: 5,
    text: "What stood out was the after-sales support — they came on site, checked the installation, and helped us document everything for our annual fire-safety review.",
  },
];
...
const TestimonialsSlider = () => {
  ...
  return (
    <section className="section-page relative overflow-hidden bg-safety-surface">
      {/* soft brand glow — separates the section without noise */}
      ...
      <div className="container-page relative z-10">
        <div className="mx-auto max-w-2xl text-center" data-aos="fade-up">
          <p className="eyebrow text-safety-red">What Clients Say</p>
          <h2 className="heading-lg mt-3 text-balance">
            Trusted by industry teams across Bangladesh
          </h2>
        ...
        >
          {/* Quote card — animated gradient border */}
          <div className="sp-testi-border relative rounded-[28px] p-[1.5px] shadow-[0_30px_80px_-40px_rgba(2,6,23,0.4)]">
           <div className="relative overflow-hidden rounded-[27px] bg-white px-6 py-10 md:px-14 md:py-14">
            <span className="pointer-events-none absolute -right-6 -top-8 select-none font-serif text-[180px] leading-none text-safety-red/[0.06] md:text-[240px]">
              ”
            </span>
            <FontAwesomeIcon
              icon={faQuoteRight}
              className="pointer-events-none absolute right-8 top-8 text-4xl text-safety-red/20 md:text-5xl"
            />

            {/* Single natural-height slide — no absolute overlap, so long quotes
                never collide with the author block on small screens. */}
            ...
            <div className="mt-8 flex items-center gap-4 border-t border-safety-border pt-6">
              ...
              <div className="min-w-0">
                <p className="truncate font-extrabold text-safety-ink">
                  {current.name}
                </p>
                <p className="truncate text-sm text-safety-muted">
          ...
          <div className="mt-7 flex flex-col items-center gap-5 sm:flex-row sm:justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm font-semibold tabular-nums text-safety-muted">
                <span className="text-safety-ink">{String(active + 1).padStart(2, "0")}</span>{" "}
              ...
              <button
                ...
                className="grid h-11 w-11 place-items-center rounded-full border border-safety-border bg-white text-safety-ink transition hover:-translate-y-0.5 hover:border-safety-red hover:text-safety-red"
              >
              ...
              <button
                ...
                className="grid h-11 w-11 place-items-center rounded-full bg-safety-red text-white transition hover:-translate-y-0.5 hover:bg-safety-red-dark"
              >
          ...
          <div className="mt-5 h-0.5 w-full overflow-hidden rounded-full bg-safety-border">
            <div
              ...
              style={{
                animation: paused
                  ? "none"
                  : `safetyTestiProgress ${AUTOPLAY_MS}ms linear forwards`,
              }}
            />
          </div>
        </div>
      ...
      <style>{`
        @keyframes safetyTestiProgress {
          from { width: 0%; }
          to { width: 100%; }
        }
        .sp-testi-border {
          background: linear-gradient(120deg, #b91c1c, #f59e0b, #fca5a5, #b91c1c);
          background-size: 300% 300%;
          animation: spTestiFlow 7s ease infinite;
        }
        @keyframes spTestiFlow {
